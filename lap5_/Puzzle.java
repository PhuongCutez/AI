...
import puzzle_8.student.PuzzleUtils;
...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		...
		for (int i = 0; i < MAX_ROW; i++) {
			for (int j = 0; j < MAX_COL; j++) {
				int titleCount = currentState.getTile(i, j);
				int titleGoal = goalState.getTile(i, j);
				if(titleCount != 0 && titleCount != titleGoal) {
					re++;
				}
			}
		}
	...
	public int computeH2(Node currentState) {
		...
		for (int i = 1; i < MAX_ROW * MAX_COL; i++) {
			int [] countPos = currentState.getLocation(i);
			int [] goalPos = currentState.getLocation(i);
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
			...
			//new position of title if move UP
			int row = whiteTile[0] + 1;
			int col = whiteTile[1];
